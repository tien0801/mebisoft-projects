...
import { ProjectStatus, PROJECT_STATUS_LABELS } from '@/features/projects/types/project.types';
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        {/* Tasks Overview - Chart placeholder */}
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="h-64 flex items-end justify-between gap-2">
            {/* Simple bar chart */}
            {['14 May', '15 May', '16 May', '17 May', '18 May', '19 May', '20 May'].map((date, index) => {
              const height = Math.random() * 80 + 20;
              return (
                <div key={date} className="flex-1 flex flex-col items-center">
                  <div className="w-full bg-cyan-500 rounded-t-lg transition-all hover:bg-cyan-600" style={{ height: `${height}%` }}></div>
                  <span className="text-xs text-gray-500 mt-2">{date.split(' ')[0]}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Top Due Projects */}
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Top Due Projects</h2>
          <div className="space-y-3">
            {topDueProjects.length > 0 ? (
              topDueProjects.map((project) => (
                <Link
                  key={project.id}
