/**
 * @file projectDetail.mock.ts
 * @description Mock data for Project Detail Dashboard
 * @author Mebisoft Team
 * @created 2025-11-29
 */

import { Attachment, Activity, Milestone } from '../types';

export interface ProjectStats {
  totalTasks: number;
  doneTasks: number;
  totalBudget: number;
  totalExpense: number;
}

export interface ProjectStatistic {
  icon: string;
  label: string;
  value: string;
  color: string;
  subtext?: boolean;
}

// Extended interface for display purposes
export interface MilestoneDisplay extends Pick<Milestone, 'id' | 'title' | 'status'> {
  tasks: number;
  color: string;
  statusColor: string;
}

export const mockProjectStats: ProjectStats = {
  totalTasks: 7,
  doneTasks: 2,
  totalBudget: 2000.00,
  totalExpense: 100.00
};

export const mockProjectStatistics: ProjectStatistic[] = [
  { icon: 'Calendar', label: 'Last 7 days task done', value: '0', color: 'bg-green-100' },
  { icon: 'Clock', label: 'Last 7 days hours spent', value: '8', color: 'bg-green-100' },
  { icon: 'Calendar', label: 'Day Left', value: '1,677/86', color: 'bg-blue-100', subtext: true },
  { icon: 'Calendar', label: 'Open Task', value: '5/7', color: 'bg-blue-100', subtext: true },
  { icon: 'Calendar', label: 'Completed Milestone', value: '1/4', color: 'bg-blue-100', subtext: true },
  { icon: 'Clock', label: 'Total project time spent', value: '81/81', color: 'bg-green-100', subtext: true }
];

export const mockProjectMilestones: MilestoneDisplay[] = [
  { 
    id: 'milestone-1',
    title: 'Hiring Individual Positions', 
    tasks: 16, 
    status: 'completed', 
    color: 'bg-green-100', 
    statusColor: 'bg-green-500' 
  },
  { 
    id: 'milestone-2',
    title: 'Communication Updates', 
    tasks: 32, 
    status: 'in_progress', 
    color: 'bg-blue-100', 
    statusColor: 'bg-blue-500' 
  },
  { 
    id: 'milestone-3',
    title: 'Design Approval', 
    tasks: 0, 
    status: 'pending', 
    color: 'bg-orange-100', 
    statusColor: 'bg-orange-500' 
  },
  { 
    id: 'milestone-4',
    title: 'Communication Updates', 
    tasks: 0, 
    status: 'cancelled', 
    color: 'bg-red-100', 
    statusColor: 'bg-red-500' 
  }
];

// Extended Activity interface for display with icon
export interface ActivityDisplay extends Activity {
  icon: string;
  detail: string;
}

export const mockActivityLogs: ActivityDisplay[] = [
  { 
    id: 'activity-1',
    userId: 'user-1',
    userName: 'Workdo',
    icon: '✚', 
    action: 'Workdo Moved the Task', 
    detail: 'The marketplace strategy', 
    timestamp: '4 years ago',
    type: 'update'
  },
  { 
    id: 'activity-2',
    userId: 'user-1',
    userName: 'Workdo',
    icon: '⟹', 
    action: 'Move Task', 
    detail: 'Workdo Moved the Task The marketplace strategy from To Do to Done', 
    timestamp: '4 years ago',
    type: 'update'
  },
  { 
    id: 'activity-3',
    userId: 'user-1',
    userName: 'Workdo',
    icon: '⟹', 
    action: 'Move Task', 
    detail: 'Workdo Moved the Task Website redesign from To Do to Done', 
    timestamp: '4 years ago',
    type: 'update'
  },
  { 
    id: 'activity-4',
    userId: 'user-1',
    userName: 'Workdo',
    icon: '⚠', 
    action: 'Create Bug', 
    detail: 'Workdo Created new bug Project1 Bug4', 
    timestamp: '4 years ago',
    type: 'create'
  },
  { 
    id: 'activity-5',
    userId: 'user-1',
    userName: 'Workdo',
    icon: '⚠', 
    action: 'Create Bug', 
    detail: 'Workdo Created new bug Project1 Bug3', 
    timestamp: '4 years ago',
    type: 'create'
  },
  { 
    id: 'activity-6',
    userId: 'user-1',
    userName: 'Workdo',
    icon: '⚠', 
    action: 'Create Bug', 
    detail: 'Workdo Created new bug Project1 Bug2', 
    timestamp: '4 years ago',
    type: 'create'
  }
];

export const mockAttachments: Attachment[] = [
  { 
    id: 'attachment-1',
    fileName: '11594448449_download.jpeg', 
    fileSize: '0.01 MB',
    fileType: 'jpg',
    uploadedBy: 'user-1',
    uploadedAt: '2021-11-26T10:00:00Z',
    projectId: 'project-1'
  },
  { 
    id: 'attachment-2',
    fileName: '11594445989_abstract-best-deals-sale-promotion-banner_44695-313.jpg', 
    fileSize: '0.1 MB',
    fileType: 'jpg',
    uploadedBy: 'user-1',
    uploadedAt: '2021-11-26T10:05:00Z',
    projectId: 'project-1'
  },
  { 
    id: 'attachment-3',
    fileName: '11594445989_abstract-best-deals-sale-promotion-banner_44695-313.jpg', 
    fileSize: '0.1 MB',
    fileType: 'jpg',
    uploadedBy: 'user-2',
    uploadedAt: '2021-11-26T10:10:00Z',
    projectId: 'project-1'
  },
  { 
    id: 'attachment-4',
    fileName: '41594448977_download.jpeg', 
    fileSize: '0.01 MB',
    fileType: 'jpg',
    uploadedBy: 'user-1',
    uploadedAt: '2021-11-26T10:15:00Z',
    projectId: 'project-1'
  },
  { 
    id: 'attachment-5',
    fileName: '41594448981_large.jpg', 
    fileSize: '0.5 MB',
    fileType: 'jpg',
    uploadedBy: 'user-3',
    uploadedAt: '2021-11-26T10:20:00Z',
    projectId: 'project-1'
  }
];

// Helper function to format currency
export const formatCurrency = (amount: number): string => {
  return `$ ${amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};
